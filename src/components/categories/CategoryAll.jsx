...
const CategoryAll = () => {
  ...
  return (
    <Card className={`rounded-2xl flex flex-col gap-4`}>
      <Card
        className={
          "p-4 bg-(--federal-blue) rounded-2xl flex justify-between items-center"
        }
      >
            ...
            return (
              <li
                ...
                className={`px-4 py-2 rounded-2xl font-bold cursor-pointer transition-all duration-500 border-2 border-white ${
                  isSelect
                    ? "text-(--federal-blue) bg-(--light-cyan) "
                    : "text-white hover:bg-(--light-cyan) hover:text-(--federal-blue)"
                }`}
                onClick={() => {
                  setIsType(link.id);
                }}
      ...
      <section className="p-4 bg-(--federal-blue) rounded-2xl overflow-y-auto">
        ...
        <div className="flex flex-col gap-4">
          {data.length > 0 ? (
            ...
            <p className="text-center col-span-full font-bold text-white">
              No hay registros...
            </p>
          )}
        </div>
